...
Add size() kernel method: To enable precondition checks in methods like remove() by providing the size of the data structure.
Add isEmpty() kernel method: To check if a data structure is empty, useful for validating preconditions in methods that require the data structure to be non-empty.
Refactor getSignals() to removeSignal(): Avoid aliasing issues by changing methods that return mutable objects to methods that modify the data structure directly.
Add contains() kernel method: To check the existence of an element in a data structure, aiding precondition checks in methods like remove() or get().
